package com.entfrm.biz.workflow.override;

import com.entfrm.biz.workflow.override.json.FlowableBpmnJsonConverter;
...
@Slf4j
@Primary
@Service("FlowableModelService")
public class FlowableModelService extends ModelServiceImpl {

    protected FlowableBpmnJsonConverter bpmnJsonConverter= new FlowableBpmnJsonConverter();
                ...
                try {
                    ...
                    for (AppModelDefinition appModelDefinition : appDefinition.getModels()) {
                        if (modelRepository.get(appModelDefinition.getId()) == null) {
                            result.getUnresolvedModels().add(new ReviveModelResultRepresentation.UnresolveModelRepresentation(appModelDefinition.getId(),
    ...
    public CmmnModel getCmmnModel(AbstractModel model) {
        ...
        try {
            ...
            log.error("Could not generate CMMN model for {}", model.getId(), e);
