...
<script>
import {listProcess, changeStatus, delProcess} from "@/api/flowable/process";
...
export default {
  ...
  methods: {
    ...
    /** 流程图按钮操作 */
    handleImg(row) {
      this.imgUrl = process.env.VUE_APP_BASE_API + `/flowable/process/resource?procInsId=` + row.deploymentId + '&procDefId=' + row.id + '&resType=image',
