  ...
  <div class="right-board">
    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <!-- 数据配置 -->
        <el-form v-show="currentTab === 'dataConfig'" :model="dataConf" ref="dataConfigForm" size="small"
                 label-width="90px">
          <el-form-item label="字段数量" prop="fieldLength" :rules="[{validator: fieldSizeValidator, trigger: 'blur' }]">
            <el-slider v-model="dataConf.fieldLength" :max="1600" :min="fieldLengthMin"
                       :marks="{320:'',640:'',960:'',1280:'',1600:''}" show-input/>
          </el-form-item>
          <el-form-item label="数据映射表" prop="dataMappingTable" :rules="[{validator: tableValidator, trigger: 'blur' }]">
            <el-input v-model="dataConf.dataMappingTable" placeholder="表名">
              <template slot="prepend">{{dataConf.dataMappingTablePrepend}}</template>
            </el-input>
          </el-form-item>
        </el-form>
      </el-scrollbar>
    ...
    <!--树形选项添加对话框-->
    <treeNode-dialog :visible.sync="dialogVisible" v-bind="$attrs" :option="option" title="添加选项" @commit="addNode"/>
    <!--图标对话框-->
    <icons-dialog :visible.sync="iconsVisible" :current="this.vGet(activeData,`attrs.${currentIconModel}`)"
                  @select="setIcon"/>
  </div>
</template>
...
import {savaModifiedOption} from "@/utils/formExpandDesign/optionPool"
import {deepClone} from "@/utils/formExpandDesign"
import {tableValidator} from "@/api/intelligent/formExtendDesign"
...
export default {
  ...
  inheritAttrs: false,
  props: ['showField', 'activeData', 'dataConf', 'option', 'fieldLengthMin'],
  data() {
    return {
      //目前激活tab
      currentTab: 'field',
  ...
  watch: {
    //监听目前控件数据发生改变时存入操作记录池
    generateActiveData: {
      handler(val, oldVal) {
        if (this.currentTab !== 'field') this.currentTab = 'field'
        //验证是否为同一组件
        if (Object.keys(oldVal).length > 0 &&
          val.config.formId === oldVal.config.formId && val.config.tagIcon === oldVal.config.tagIcon) {
          savaModifiedOption(val)
          console.info("保存修改数据", val)
        }
      },
      deep: true
    }
  ...
  computed: {
    //返回新对象目的让监听器重新创建数据源[实现新旧不一样]
    generateActiveData() {
      return deepClone(this.activeData)
    },
    //elementUi组件操作文档
    documentLink() {
  ...
  methods: {
    //字段数量校验
    fieldSizeValidator(rule, value, callback) {
      if (this.option === 1) {
        callback()
        return
      }
      if (value === 0) {
        callback(new Error('字段数量不能为0'))
        return
      }
      callback()
    },
    //数据映射表是否重复ajax校验
    async tableValidator(rule, value, callback) {
      if (this.option === 1) {
        callback()
        return
      }
      if (!value) {
        callback(new Error('请输入数据映射表'))
        return
      }
      let have = 0
      await tableValidator(this.dataConf.dataMappingTablePrepend.concat(value)).then(response => have = response.data)
      if (have !== 0) {
        callback(new Error('该表名数据库已经存在!'))
      } else {
        callback()
      }
    },
