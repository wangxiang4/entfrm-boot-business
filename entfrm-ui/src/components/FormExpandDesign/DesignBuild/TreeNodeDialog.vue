...
import {getTreeNodeData, saveTreeNodeData} from '@/utils/formExpandDesign/db'
...
export default {
  ...
  watch: {
    ...
    treeNodeData: {
      handler(val) {
        if (this.option === 1) {
          this.$store.dispatch("treeNodeData/setFormExtendTreeNodeData", val)
        } else {
          this.treeNodeDataDB[this.formExtendId] = val
          saveTreeNodeData(this.treeNodeDataDB)
        }
      },
      deep: true
    },
  ...
  methods: {
    ...
    onOpen() {
      if (this.option === 1) {
        ...
        //初始化节点ID
        this.treeNodeDataDB = getTreeNodeData()
        let temp = this.treeNodeDataDB[this.formExtendId]
        if (temp && Object.keys(temp).length > 0) {
          this.treeNodeData = temp
          this.id = this.treeNodeData[this.idGlobal] || 100
        }
      }
      this.formData = {
