        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="归属机构" prop="deptId">
                <treeselect v-model="form.deptId" :options="deptOptions" :normalizer="normalizer" placeholder="请选择归属机构" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="手机号码" prop="phone">
...
<script>
import { listUser, getUser, delUser, addUser, editUser, exportUser, resetPwd, changeStatus, importTemplate } from "@/api/system/user";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/system/user/importUser"
      },
      // 查询参数
      queryParams: {
        current: 1,
      ...
      rules: {
        ...
        deptId: [
          { required: true, message: "归属机构名称不能为空", trigger: "blur" }
        ],
        password: [
  ...
  created() {
    ...
    this.getByKey("user.initPassword").then(response => {
  ...
  methods: {
    ...
    handleEdit(row) {
      ...
      getUser(id).then(response => {
        ...
        this.roleOptions = response.data.extend;
        this.form.roles = response.data.result.roles;
        this.open = true;
        this.title = "修改用户";
        this.form.password = "";
      });
